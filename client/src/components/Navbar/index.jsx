...
const MobileNav = ({ user, signIn, signUp, isDropdownOpen, setIsDropdownOpen }) => {
  const SignIn = () => {
    signIn();
...
const LargeNav = ({ user, signIn, signUp,isDropdownOpen, setIsDropdownOpen }) => {
  ...
  return (
    <div className="w-full items-center justify-between hidden lg:flex px-10">
      ...
      <div className="flex items-center gap-3 relative">
        {user?.fullName ? (
          <>
            ...
            {isDropdownOpen && (
              <div className="absolute shadow-lg py-3 -bottom-14 -right-10 w-36 z-20 flex flex-col gap-2 bg-white border border-gray-200">
                <button onClick={SignOut}>Sign Out</button>
              </div>
            )}
          </>
